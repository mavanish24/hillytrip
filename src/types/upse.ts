...
export interface UPSECoupon {
  ...
  applicableModules?: string[]; // e.g. ['taxi', 'homestay', 'activity', 'guide', 'all']
  validFrom: string;
  validUntil: string;
  usageLimit?: number;
  usedCount: number;
...
export interface UPSEInvoiceItem {
  description: string;
  quantity: number;
  unitPrice: number;
  totalAmount: number;
...
export interface UPSEWalletTransaction {
  id: string;
  walletId: string;
  userId: string;
  type: 'credit' | 'debit';
  amount: number;
  currency: string;
  purpose: 'topup' | 'booking_payment' | 'refund' | 'cashback' | 'withdrawal';
  referenceId?: string; // paymentId or bookingId
  description: string;
  createdAt: string;
}
