...
import { Camera, UploadCloud, FileImage, Sparkles, LogIn, Loader2, CheckCircle2, AlertCircle, Info, Heart, Download, ChevronLeft, ChevronRight, Share2, MessageSquare, ZoomIn } from 'lucide-react';
...
function LightboxImageViewer(props: {
  url: string;
  altText?: string;
  caption?: string;
  onNext: () => void;
...
export default function ImageGallerySystem({
  ...
  const handlePhotoCommentSubmit = async () => {
    ...
    try {
      await onAddComment(selectedPhotoForViewer.id, 'photo', localPhotoCommentText.trim());
  ...
  useEffect(() => {
    if (initialSelectedPhotoUrl) {
      ...
      if (matchedPhoto) {
        setSelectedPhotoForViewer(matchedPhoto);
        setEditingCaptionText(matchedPhoto.caption || '');
        setEditingAltTextText(matchedPhoto.altText || '');
        setIsEditingCaption(false);
      }
      if (onClearInitialSelectedPhotoUrl) {
        onClearInitialSelectedPhotoUrl();
      }
    }
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAdmin) {
        ...
        const metaPayload = {
          destinationId: entityType === 'destination' ? entityId : (parentDestinationId || null),
        ...
        const response = await fetch('/api/photo-contributions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    <div className="bg-slate-50 rounded-3xl p-6 md:p-8 border border-slate-200 mt-12 shadow-xs">
      ...
      {selectedPhotoForViewer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md animate-fade-in">
          ...
          <div className="relative w-full max-w-5xl xl:max-w-6xl bg-white dark:bg-slate-950 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row max-h-[92vh] z-10 border border-slate-200 dark:border-slate-800">
            {/* Close icon */}
            <button 
              type="button"
            ...
            {/* Left Portion: Cinematic Image Viewer with touch swipe support */}
            <LightboxImageViewer
              url={selectedPhotoForViewer.url || DEFAULT_HOMESTAY_IMAGE}
              altText={selectedPhotoForViewer.altText}
                ...
                <div className="space-y-4 pt-2 border-t border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-4">
                    {onToggleLike && (
                      <button
                        type="button"
                        onClick={() => onToggleLike(selectedPhotoForViewer.id, 'photo')}
                        className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300 hover:text-rose-600 transition cursor-pointer"
                        title="Like photo"
                      >
                        <Heart 
                          className={`w-4 h-4 ${
                            user && likes.some((l) => l.id === `${user.uid}_${selectedPhotoForViewer.id}`)
                              ? 'fill-rose-600 text-rose-600'
                              : 'text-slate-500 hover:text-rose-600'
                          }`}
                        />
                        <span className="text-xs font-bold">
                          {likes.filter((l) => l.contentId === selectedPhotoForViewer.id).length}
                        </span>
                      </button>
                    )}
                    ...
                    <div className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300">
                      ...
                      <span className="text-xs font-bold">
                        {comments.filter(c => c.contentId === selectedPhotoForViewer.id && c.contentType === 'photo').length} Comments
                      </span>
                    </div>
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const directUrl = `${window.location.origin}/photo/${selectedPhotoForViewer.id}`;
                        navigator.clipboard.writeText(directUrl);
                        setNotification({ type: 'success', message: '📋 Image link copied! Share with friends!' });
                      }}
                      className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300 hover:text-indigo-600 transition cursor-pointer ml-auto"
                      title="Copy photo link"
                    >
                      <Share2 className="w-4 h-4 text-slate-550" />
                      <span className="text-xs font-bold">Share</span>
                    </button>
                  </div>

                  {/* Comments list for this specific photo */}
                  <div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3 border border-slate-100 dark:border-slate-850 space-y-3">
                    <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 block font-mono">
                      Photo Comments ({comments.filter(c => c.contentId === selectedPhotoForViewer.id && c.contentType === 'photo').length})
                    </span>
                    
                    <div className="max-h-[120px] overflow-y-auto space-y-2 pr-1 scrollbar-thin">
                      {comments.filter(c => c.contentId === selectedPhotoForViewer.id && c.contentType === 'photo').length === 0 ? (
                        <p className="text-[11px] text-slate-400 italic">No comments on this photo yet.</p>
                      ) : (
                        comments.filter(c => c.contentId === selectedPhotoForViewer.id && c.contentType === 'photo').map(c => (
                          <div key={c.id} className="text-xs border-b border-slate-100 dark:border-slate-800/50 pb-1.5 last:border-0 last:pb-0">
                            <div className="flex items-center justify-between">
                              <span className="font-bold text-slate-700 dark:text-slate-300">{c.userName}</span>
                              {((user && user.uid === c.userId) || isAdmin) && onDeleteComment && (
                                <button
                                  type="button"
                                  onClick={() => onDeleteComment(c.id)}
                                  className="text-[9px] text-red-500 hover:text-red-600 cursor-pointer"
                                >
                                  Delete
                                </button>
                              )}
                            </div>
                            <p className="text-slate-600 dark:text-slate-400 font-sans mt-0.5">{c.text}</p>
                          </div>
                        ))
                      )}
                    </div>

                    {/* Add Photo Comment input */}
                    {onAddComment && (
                      <div className="pt-1.5 border-t border-slate-200/40 dark:border-slate-800/40">
                        {user ? (
                          <div className="flex gap-2">
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                  e.preventDefault();
                                  handlePhotoCommentSubmit();
                                }
                              }}
                              className="flex-grow text-xs bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg p-2 dark:text-white focus:outline-none focus:border-emerald-500"
                            />
                            <button
                              type="button"
                              onClick={handlePhotoCommentSubmit}
                              className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs px-3 rounded-lg transition-colors cursor-pointer"
                            >
                              Post
                            </button>
                          </div>
                          ...
                          <p className="text-[10px] text-slate-450 text-center">
                            🔐 <button type="button" onClick={() => { setSelectedPhotoForViewer(null); onLogin(); }} className="text-emerald-600 dark:text-emerald-400 font-bold hover:underline">Sign in to comment</button>
                          </p>
                        )}
                      </div>
                    )}
