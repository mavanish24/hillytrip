...
import { 
  ...
  ShieldAlert, Radio, AlertCircle, Sparkles, Activity
} from 'lucide-react';
import { AppNotification } from '../types';
import AdminCommunicationDashboard from './communication/AdminCommunicationDashboard';
...
export default function AdminNotificationsTab({ adminEmail }: AdminNotificationsTabProps) {
  const [activeEngineMode, setActiveEngineMode] = useState<'communication_engine' | 'broadcast_alerts'>('communication_engine');
  ...
  const getAuthHeaders = () => {
    const token = localStorage.getItem('hillytrip_admin_token') || localStorage.getItem('token') || '';
    const headers: Record<string, string> = {
      'x-admin-email': adminEmail || 'admin@hillytrip.com'
    };
    if (token) {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      let data: any = null;
      const res = await fetch(`/api/admin/app-notifications?email=${encodeURIComponent(adminEmail || 'admin@hillytrip.com')}`, {
        headers: getAuthHeaders()
      });
      if (res.ok) {
        data = await res.json();
      } else {
        // Fallback to public published notifications
        const fallbackRes = await fetch('/api/app-notifications');
        if (fallbackRes.ok) {
  ...
  const handleSaveNotification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingNotif) {
        // Edit flow
        const res = await fetch(`/api/admin/app-notifications/${editingNotif.id}?email=${encodeURIComponent(adminEmail || 'admin@hillytrip.com')}`, {
          method: 'PUT',
          headers: { 
  ...
  const handleStatusUpdate = async (id: string, newStatus: 'published' | 'rejected' | 'pending' | 'draft') => {
    try {
      const res = await fetch(`/api/admin/app-notifications/${id}?email=${encodeURIComponent(adminEmail || 'admin@hillytrip.com')}`, {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json',
          ...getAuthHeaders()
    ...
    <div id="admin-notifications-panel" className="space-y-6">
      ...
      <div className="flex items-center justify-between p-2 bg-slate-100 dark:bg-slate-800 rounded-2xl">
        <div className="flex items-center gap-1 w-full">
          <button
            onClick={() => setActiveEngineMode('communication_engine')}
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-black transition flex items-center justify-center gap-2 ${
              activeEngineMode === 'communication_engine'
                ? 'bg-emerald-600 text-white shadow-md'
                : 'text-slate-600 dark:text-slate-300 hover:text-slate-900'
            }`}
          >
            <Activity className="w-4 h-4" /> Phase 2B Communication Engine
          </button>
          <button
            onClick={() => setActiveEngineMode('broadcast_alerts')}
            className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-black transition flex items-center justify-center gap-2 ${
              activeEngineMode === 'broadcast_alerts'
                ? 'bg-emerald-600 text-white shadow-md'
                : 'text-slate-600 dark:text-slate-300 hover:text-slate-900'
            }`}
          >
            <Bell className="w-4 h-4" /> App Broadcast Alerts Catalogue
          </button>
        </div>
      </div>
