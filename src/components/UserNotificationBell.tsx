...
export default function UserNotificationBell({ userId: propUserId, role: propRole, className }: UserNotificationBellProps) {
  ...
  const fetchAggregatedNotifications = async () => {
    ...
    try {
      ...
      console.warn('UNE: Notification sync temporary fallback:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchUnePreferences = async () => {
    ...
    try {
      ...
      console.warn('UNE: Preferences sync temporary fallback:', err);
    }
  };
  ...
  return (
    <div className="relative inline-block" ref={dropdownRef} id="hillytrip-une-bell-root">
      ...
      <button
        ...
        id="une-bell-btn"
        className={`relative ${className ? className : 'w-11 h-11'} rounded-full border transition-all duration-300 ease-out focus:outline-none cursor-pointer flex items-center justify-center backdrop-blur-[24px] shadow-sm hover:-translate-y-1 hover:scale-[1.05] hover:shadow-lg hover:shadow-amber-500/10 active:scale-95 ${
          isOpen 
            ? 'bg-amber-500 border-amber-400 text-slate-950 shadow-md shadow-amber-500/20' 
            : 'bg-[#121826]/55 border-white/[0.08] text-slate-200 hover:text-white hover:border-white/20'
        }`}
        title="Notifications"
