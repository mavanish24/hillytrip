...
const defaultSettings: SiteSettings = {
  ...
  hero_video_url: '',
  hero_image_url: 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/common/hero.png',
...
export const BrandingProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      const cached = localStorage.getItem('hillytrip_site_settings');
      if (cached) {
        try {
      ...
      // 2. Fetch latest from API endpoint if available and valid JSON
      ...
      if (res.ok && contentType && contentType.includes('application/json')) {
        ...
        if (data && typeof data === 'object') {
          const fixUrl = (url?: string) => {
            if (!url || url.includes('hillytrip_logo.png') || url === '/hillytrip_logo.jpg') return '/hillytrip_logo.jpg?v=2';
            return url;
          };
          const sanitizedData = {
            ...data,
            desktop_logo_url: fixUrl(data.desktop_logo_url),
            mobile_logo_url: fixUrl(data.mobile_logo_url),
            footer_logo_url: fixUrl(data.footer_logo_url),
            white_logo_url: fixUrl(data.white_logo_url),
            dark_logo_url: fixUrl(data.dark_logo_url),
            favicon_url: fixUrl(data.favicon_url),
            app_icon_url: fixUrl(data.app_icon_url),
            apple_touch_icon_url: fixUrl(data.apple_touch_icon_url),
            android_pwa_icon_url: fixUrl(data.android_pwa_icon_url),
          };
          setSettings(sanitizedData);
          localStorage.setItem('hillytrip_site_settings', JSON.stringify(sanitizedData));
        }
      }
    } catch (err) {
      // Gracefully fall back to default settings without crashing
      console.warn('Site branding using default settings.');
    } finally {
      setIsLoading(false);
    }
  };
