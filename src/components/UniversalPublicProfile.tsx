...
function ProfileLightboxImageViewer({
  url,
  caption,
  onNext,
  onPrev,
  ...
  return (
    <div 
      className="relative flex-1 flex items-center justify-center overflow-hidden touch-pan-y select-none"
      {...touchHandlers}
    >
      <button
...
const PRESEEDED_BUSINESSES: Record<string, any> = {
  'himalayan-homestay': {
    ...
    gallery: [
      { url: 'undefined', category: 'rooms', caption: 'Scenic Sunrise from Wood Balcony' },
      { url: 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/destinations/Snow%20Mountains.png', category: 'surroundings', caption: 'Mt. Kanchenjunga Range view' },
      { url: 'undefined', category: 'surroundings', caption: 'Evening outdoor fireplace setup' },
      { url: 'undefined', category: 'rooms', caption: 'Rustic Colonial Bedroom style' }
    ],
    trust: {
      healthScore: 98,
  ...
  'green-valley-taxi': {
    ...
    gallery: [
      { url: 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/taxi-stands/Major%20Hill%20Town%20Taxi%20Stand.png', category: 'fleet', caption: 'Our Clean Innova Fleet at Rohtang Pass' },
      { url: 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/taxi-stands/Mountain%20Road.png', category: 'fleet', caption: 'Cozy leather seating interior detail' },
      { url: 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/taxi-stands/Himalayan%20Taxi%20Stand.png', category: 'surroundings', caption: 'Spectacular hairpin curves on mountain roads' }
    ],
    trust: {
      healthScore: 99,
  ...
  'summit-trekking': {
    ...
    gallery: [
      { url: 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/destinations/Snow%20Mountains.png', category: 'trek', caption: 'High Ridge Trek overlooking glaciers' },
      { url: 'undefined', category: 'trek', caption: 'Base camp star-gazing dome setups' },
      { url: 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/attractions/Trek%20%20Hiking.png', category: 'trek', caption: 'Climbing team crossing high altitude passes' }
    ],
    trust: {
      healthScore: 97,
...
export default function UniversalPublicProfile({
  ...
  const activeBusiness = useMemo(() => {
    ...
    if (foundHomestay) {
      return {
        ...
        gallery: (foundHomestay.images && foundHomestay.images.length > 0) 
          ...
          : [{ url: 'undefined', category: 'rooms', caption: 'Scenic Window View' }],
        trust: {
          healthScore: 96,
    ...
    return {
      ...
      gallery: [
        { url: 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/destinations/Snow%20Mountains.png', category: 'rooms', caption: 'Scenic Mountains background' },
        { url: 'undefined', category: 'rooms', caption: 'Homestead Exterior view' }
      ],
      trust: {
        healthScore: 95,
  ...
  // Compute active dynamic reviews (real posted reviews from Supabase/API)
  const activeReviews = useMemo(() => {
    const list = [...postedReviews];
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-24" id="universal-profile-engine-root">
      ...
      <div className="relative bg-white dark:bg-slate-950 rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-850 shadow-sm">
        ...
        <div className="relative h-64 md:h-80 w-full overflow-hidden bg-slate-100 dark:bg-slate-900">
          <img 
            src={activeBusiness.gallery?.[0]?.url || 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/destinations/Snow%20Mountains.png'} 
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="flex flex-col md:flex-row items-start md:items-end gap-5">
              ...
              <div className="w-24 h-24 md:w-32 md:h-32 bg-white dark:bg-slate-900 border-4 border-white dark:border-slate-950 rounded-2xl overflow-hidden shadow-lg flex-shrink-0">
                <img 
                  src={activeBusiness.gallery?.[1]?.url || activeBusiness.gallery?.[0]?.url || 'undefined'} 
                  alt={activeBusiness.name}
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  <div 
                    ...
                    style={{ 
                      backgroundImage: `url(${activeBusiness.gallery?.[0]?.url || 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/destinations/Snow%20Mountains.png'})`,
      ...
      <AnimatePresence>
        {lightboxImageIndex !== null && (
          <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-md flex flex-col justify-between p-6">
            ...
            <ProfileLightboxImageViewer
              url={filteredGallery[lightboxImageIndex]?.url}
              caption={filteredGallery[lightboxImageIndex]?.caption}
              onNext={() => setLightboxImageIndex(prev => prev !== null ? (prev + 1) % filteredGallery.length : null)}
              onPrev={() => setLightboxImageIndex(prev => prev !== null ? (prev - 1 + filteredGallery.length) % filteredGallery.length : null)}
            />
