...
function OperatorLightboxViewer({
  url,
  category,
  index,
  total,
  ...
  return (
    <div 
      className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 touch-pan-y select-none overflow-hidden"
      onClick={onClose}
      {...touchHandlers}
    >
      <div className="absolute top-4 left-4 text-xs font-mono font-bold text-white/80">
        {index + 1} / {total} {category && `• ${category.toUpperCase()}`}
      </div>
      <button 
        onClick={onClose} 
        className="absolute top-4 right-4 bg-slate-900 border border-slate-800 text-white p-2 rounded-full cursor-pointer z-20"
      >
        <X className="w-5 h-5" />
      </button>
      ...
      <img 
        ...
        className="max-w-full max-h-[85vh] object-contain rounded-xl shadow-2xl border border-slate-800 select-none pointer-events-none" 
      />
    </div>
  );
}
...
export default function PublicOperatorProfile({ operatorId, onClose, onNavigate }: PublicOperatorProfileProps) {
            ...
            if (foundBooking) {
              ...
              setOperatorUser({
                ...
                taxiOperatorDetails: {
                  ...
                  gallery: [
                    { url: '/images/hillytrip/taxi-transit.svg', category: 'fleet' },
                    { url: '/images/hillytrip/taxi-transit.svg', category: 'fleet' }
                  ],
                  services: {
          ...
          'partner_hillytrip': {
            ...
            taxiOperatorDetails: {
              ...
              gallery: [
                { url: '/images/hillytrip/taxi-transit.svg', category: 'fleet' },
                { url: '/images/hillytrip/taxi-transit.svg', category: 'fleet' },
                { url: '/images/hillytrip/taxi-transit.svg', category: 'office' }
              ],
              services: {
                airportTransfer: true,
                railwayPickup: true,
  ...
  const banner = details.coverImage || '/images/hillytrip/himalayan-landscape.svg';
    ...
    <div className="max-w-4xl mx-auto space-y-6" id="public_operator_profile">
      ...
      <AnimatePresence>
        {lightboxIndex !== null && filteredGallery[lightboxIndex] && (
          <OperatorLightboxViewer
            url={filteredGallery[lightboxIndex].url}
            category={filteredGallery[lightboxIndex].category}
            index={lightboxIndex}
            total={filteredGallery.length}
            onClose={() => setLightboxIndex(null)}
            onNext={() => setLightboxIndex((lightboxIndex + 1) % filteredGallery.length)}
            onPrev={() => setLightboxIndex((lightboxIndex - 1 + filteredGallery.length) % filteredGallery.length)}
          />
        )}
        {lightboxImage && (
          <motion.div 
