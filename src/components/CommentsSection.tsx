        ...
        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
          {filteredComments.map((comment) => (
            ...
            >
              <div 
                onClick={() => {
                  window.location.hash = `#/traveller/${comment.userId || comment.userName}`;
                }}
                className="w-9 h-9 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center shrink-0 font-bold text-slate-600 text-xs cursor-pointer hover:ring-2 hover:ring-emerald-500 transition-all"
                title={`View ${comment.userName}'s traveler profile`}
              >
              ...
              <div className="flex-grow space-y-1">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <button
                    ...
                    className="text-sm font-bold text-slate-800 hover:text-emerald-600 hover:underline cursor-pointer text-left"
                  >
                    {comment.userName}
                  </button>
