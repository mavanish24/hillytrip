...
function AttractionDetailPageInner({
  ...
  const handleReplySubmit = async (commentId: string) => {
    ...
    try {
      const commentToUpdate = (comments || []).find(c => c && c.id === commentId);
  ...
  const isLiked = useMemo(() => user && (likes || []).some(l => l && l.id === `${user.uid}_${attraction.id}`), [user, likes, attraction?.id]);
  const isSaved = isItemSaved ? isItemSaved(attraction.id) : false;
  const likesCount = useMemo(() => (likes || []).filter(l => l && l.contentId === attraction.id).length, [likes, attraction?.id]);
  
  const attrComments = useMemo(() => (comments || []).filter(c => c && c.contentId === attraction.id && c.contentType === 'attraction'), [comments, attraction?.id]);

  // Auto-generate statistics for Explore Destination Card
  const destinationAttractionsCount = useMemo(() => (attractions || []).filter(a => a && a.destinationId === attraction.destinationId).length, [attractions, attraction?.destinationId]);
  const destinationHomestaysCount = useMemo(() => (homestays || []).filter(h => h && h.destinationId === attraction.destinationId).length, [homestays, attraction?.destinationId]);

  // Filter Nearby Stays
  const nearbyStays = useMemo(() => (homestays || [])
    .filter(h => h && h.destinationId === attraction.destinationId)
    .slice(0, 6), [homestays, attraction?.destinationId]);

  // Filter Regional Transport/Drivers
  const regionalDrivers = useMemo(() => (drivers || [])
    .filter(d => d && d.status === 'Approved')
    .slice(0, 3), [drivers]);

  // Explore More: Similar Category Attractions
  const similarAttractions = useMemo(() => (attractions || [])
    .filter(a => a && a.category === attraction.category && a.id !== attraction.id)
    .slice(0, 6), [attractions, attraction?.category, attraction?.id]);

  // Explore More: Trending Attractions
  const trendingAttractions = useMemo(() => (attractions || [])
    .filter(a => a && a.id !== attraction.id)
    .slice(0, 6), [attractions, attraction?.id]);
  ...
  const basePhotos = useMemo(() => [
    ...
    ...(activePhotos || []).map((photo) => {
      const photoLikes = (likes || []).filter((l) => l && l.contentId === photo.id).length;
      const photoCommentsCount = (comments || []).filter((c) => c && c.contentId === photo.id && c.contentType === 'photo').length;
      return {
        id: photo.id,
        url: photo.url,
        isStatic: false,
  ...
  ], [attraction?.gallery, activePhotos, likes, comments]);

  // Apply filters: liked, latest, popular
  const filteredPhotos = useMemo(() => [...basePhotos].sort((a, b) => {
  ...
  }), [basePhotos, galleryFilter]);

  const checkDuplicatePhoto = (file: File) => {
    const cleanName = file.name.split('.')[0].toLowerCase();
    const hasStaticDuplicate = (attraction.gallery || []).some(url => url && url.toLowerCase().includes(cleanName));
    const hasActiveDuplicate = (activePhotos || []).some(photo => photo && ((photo.caption && photo.caption.toLowerCase().includes(cleanName)) || (photo.url && photo.url.toLowerCase().includes(cleanName))));
  ...
  const breadcrumbItems = [
    ...
    ...(destination ? [{ name: destination.name, path: `/destination/${toSlug(destination.id || destination.name)}` }] : []),
    { name: attraction.name, path: `/attraction/${toSlug(attraction.id || attraction.name)}` }
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 pb-20 animate-fade-in font-sans">
      <SEOBreadcrumbs navigate={navigate} items={breadcrumbItems} />
      ...
      <section className="relative h-[480px] md:h-[580px] lg:h-[650px] w-full overflow-hidden text-white flex flex-col justify-end">
        ...
        >
          ...
          />
          <div className={`absolute inset-0 bg-gradient-to-b ${weatherConfigs[weatherTheme]?.overlayGradient || 'from-transparent via-transparent to-slate-950/45'} transition-all duration-700 pointer-events-none`} />
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pb-12 sm:pb-16 text-left flex flex-col items-start gap-4">
          <div className="space-y-3.5 max-w-5xl w-full">
            ...
            <div className="mt-3 max-w-4xl border-l-2 border-emerald-500/50 pl-4 py-0.5">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-7 sm:-mt-9 relative z-30">
        <div className="bg-white dark:bg-slate-900 rounded-2xl md:rounded-3xl shadow-xl border border-slate-200/90 dark:border-slate-800 p-2 sm:p-3 flex items-center justify-between w-full mx-auto gap-1">
      ...
      {/* MAIN CONTENT AREA IN FULL-WIDTH CONTAINER */}
      <main className="w-full mt-8">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
              ...
              {filteredPhotos.length === 0 ? (
                ...
                <UniversalCarousel
                  ...
                  renderItem={(photo, idx) => {
                    const likedByUser = (likes || []).some((l) => l && l.contentId === photo.id && l.userId === user?.uid);
                    return (
                      <motion.div 
            ...
            {/* 4. HOW TO REACH (DYNAMIC API & PROXIMITY DRIVEN) */}
            <HowToReachSection
            ...
            <section id="nearby-attractions-section" className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-xs border border-slate-200/60 dark:border-slate-800/80 text-left">
              ...
              {(() => {
                const list = (attractions || []).filter(a => a && a.destinationId === attraction.destinationId && a.id !== attraction.id);
                if (list.length === 0) {
                  return (
            ...
            {/* 6. UNIVERSAL PROXIMITY NETWORK (Geo-Based Nearest Villages, Stays, Attractions, Taxis) */}
            ...
            <section className="bg-gradient-to-br from-slate-950 to-indigo-950 text-white rounded-3xl p-6 sm:p-8 border border-indigo-900/60 shadow-xl text-left relative overflow-hidden">
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
                <div className="lg:col-span-5 space-y-3">
                  <Compass className="w-9 h-9 text-emerald-400 mb-2 animate-spin-slow" />
                  <h4 className="font-extrabold text-2xl text-white">Need local travel assistance?</h4>
                  <p className="text-slate-350 text-xs sm:text-sm leading-relaxed">
                    Submit a bespoke mountain planning lead. Our regional coordinators will connect with you on WhatsApp to customize verified homestays, private cabs, and complete high-altitude itineraries.
                  </p>
                </div>
                
                <div className="lg:col-span-7">
                  {attrLeadSuccess ? (
                    <div className="bg-emerald-950/40 border border-emerald-500/25 p-6 rounded-2xl text-center animate-fade-in">
                      <CheckCircle className="w-9 h-9 text-emerald-400 mx-auto mb-2" />
                      <p className="text-base font-extrabold text-emerald-300">Travel Lead Registered!</p>
                      ...
                      <button 
                        ...
                        className="text-xs text-emerald-400 underline font-black mt-4 hover:text-emerald-300 transition cursor-pointer uppercase tracking-wider"
                      >
                        Submit New Enquiry
                      </button>
                    </div>
                  ) : (
                    <form onSubmit={handleAttractionLeadSubmit} className="space-y-4 text-left">
            ...
            <section id="traveler-discussions-section" className="text-left space-y-6">
        ...
        <section id="traveler-reviews-section" className="text-left space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 shadow-xs border border-slate-200 dark:border-slate-800">
            <div className="border-b border-slate-100 dark:border-slate-800 pb-4 mb-6">
              <h3 className="font-extrabold text-2xl text-slate-900 dark:text-white flex items-center gap-2">
        ...
        <section className="mt-8 text-left space-y-12">
      ...
      {/* Lightbox / Traveller Moment Viewer Modal */}
      {selectedPhotoForViewer && (
        <TravellerMomentViewer
          moments={(filteredPhotos || []).map((photo, idx) => ({
            id: photo.id,
            url: safeSrc(photo.url),
            caption: photo.caption || 'Traveler Share',
            travellerName: photo.uploadedBy || 'HillyTrip Traveler',
            travellerId: 'usr-anon',
