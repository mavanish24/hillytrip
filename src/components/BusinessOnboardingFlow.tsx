...
export default function BusinessOnboardingFlow({
  ...
  const [selectedBusinessType, setSelectedBusinessType] = useState<string | null>(() => {
    if (currentPath === '/register/homestay') return 'homestay';
    if (currentPath === '/register/driver' || currentPath === '/become-taxi-operator' || currentPath === '/register/taxi-operator') return 'taxi_operator';
    return localStorage.getItem('hillytrip_selected_business_type') || 'homestay';
  });

  // Flow state: 'selection' or 'onboarding'
  const [flowState, setFlowState] = useState<'selection' | 'onboarding'>(() => {
    if (currentPath === '/register' || currentPath === '/become-partner') return 'selection';
    return 'onboarding';
  });

  React.useEffect(() => {
    // Set business type and flow state based on path
    if (currentPath === '/register/homestay') {
      setSelectedBusinessType('homestay');
      setFlowState('onboarding');
      localStorage.setItem('hillytrip_selected_business_type', 'homestay');
    } else if (currentPath === '/register/driver' || currentPath === '/become-taxi-operator' || currentPath === '/register/taxi-operator') {
      setSelectedBusinessType('taxi_operator');
      setFlowState('onboarding');
      localStorage.setItem('hillytrip_selected_business_type', 'taxi_operator');
    } else if (currentPath === '/register' || currentPath === '/become-partner') {
  ...
  if (flowState === 'onboarding' && selectedBusinessType) {
    if (!user) {
      const isTaxi = selectedBusinessType === 'taxi_operator';
      return (
        <div className="max-w-md mx-auto my-16 bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-xl text-center animate-fade-in">
          <div className="w-14 h-14 bg-amber-500/10 text-amber-500 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-amber-500/20">
            {isTaxi ? <Car className="w-7 h-7" /> : <Building2 className="w-7 h-7" />}
          </div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">
            {isTaxi ? 'Become a Taxi Partner' : `Become a ${currentItem?.name || 'Partner'}`}
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
            {isTaxi 
              ? 'Please sign in to register your taxi fleet and receive direct traveler booking requests on HillyTrip.'
              : `Please sign in to register your ${currentItem?.name || 'business'} and receive direct traveler booking requests on HillyTrip.`}
          </p>
          <button
            onClick={() => {
              localStorage.setItem('hillytrip_selected_business_type', selectedBusinessType);
              if (onOpenLoginModal) onOpenLoginModal();
            }}
            className="mt-6 w-full bg-amber-500 hover:bg-amber-400 text-slate-950 font-black py-3 px-4 rounded-xl transition shadow-md flex items-center justify-center gap-2 cursor-pointer"
          >
            <span>Sign In / Authenticate to Continue</span>
