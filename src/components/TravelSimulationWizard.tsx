...
export default function TravelSimulationWizard({
  ...
  const getCuratedResults = () => {
    ...
    // Filter actual homestays nested in recommended destinations by coordinates or proximity
    const curatedHomestays = homestays.filter(h => {
      return finalDests.some(d => {
        if (d.latitude && d.longitude && h.latitude && h.longitude) {
          return calculateHaversineDistanceKm(d.latitude, d.longitude, h.latitude, h.longitude) <= 25;
        }
        return (h.district && d.district && h.district.toLowerCase() === d.district.toLowerCase()) ||
               (h.village_name && d.name && h.village_name.toLowerCase().includes(d.name.toLowerCase()));
      });
    }).slice(0, 3);
    ...
    // Filter scenic spots / attractions in recommended destinations by coordinates or proximity
    const curatedAttractions = attractions.filter(a => {
      return finalDests.some(d => {
        if (d.latitude && d.longitude && a.latitude && a.longitude) {
          return calculateHaversineDistanceKm(d.latitude, d.longitude, a.latitude, a.longitude) <= 25;
        }
        return (a.district && d.district && a.district.toLowerCase() === d.district.toLowerCase()) ||
               (a.village_name && d.name && a.village_name.toLowerCase().includes(d.name.toLowerCase()));
      });
    }).slice(0, 4);
                ...
                <div className="space-y-3.5">
                  ...
                  <div className="space-y-3">
                    {results.destinationsList.map((dest) => (
                      <div 
                        key={dest.id}
                        onClick={() => onNavigate(`#/village/${getItemSlug(dest)}`)}
                ...
                <div className="space-y-3.5">
                  ...
                  <div className="space-y-3">
                    {results.homestaysList.map((homestay) => (
                      <div 
                        key={homestay.id}
                        onClick={() => onNavigate(`#/homestay/${getItemSlug(homestay)}`)}
                        className="group flex gap-3.5 bg-slate-850/50 hover:bg-slate-800/80 border border-slate-800 hover:border-emerald-500/35 p-3.5 rounded-2xl transition duration-200 cursor-pointer"
                      >
                        <div className="w-20 h-20 rounded-xl overflow-hidden bg-slate-850 shrink-0 relative">
                          <img 
                            src={homestay.images && homestay.images[0] ? homestay.images[0] : 'undefined'} 
                            alt={homestay.name} 
                            className="w-full h-full object-cover group-hover:scale-105 transition"
                          />
                        </div>
