  ...
  useEffect(() => {
    ...
    const loadForecast = async () => {
      ...
      try {
        ...
        const openMeteoUrl = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_sum,wind_speed_10m_max&timezone=auto`;
        ...
        if (openMeteoData && openMeteoData.daily) {
          ...
          for (let i = 0; i < 5; i++) {
            ...
            const maxTemp = daily.temperature_2m_max[i] ?? 18;
            const minTemp = daily.temperature_2m_min[i] ?? 8;
            const rainSum = daily.precipitation_sum[i] ?? 0;
            const wind = daily.wind_speed_10m_max[i] ?? 12;
            const humidity = (daily.relative_humidity_2m_max && daily.relative_humidity_2m_max[i]) ? daily.relative_humidity_2m_max[i] : 65;
