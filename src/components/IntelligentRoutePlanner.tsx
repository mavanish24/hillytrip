...
const DEFAULT_COVER = 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/taxi-stands/Major%20Hill%20Town%20Taxi%20Stand.png';
...
export default function IntelligentRoutePlanner({
  ...
  const addRecentSearch = (fromName: string, toName: string) => {
    ...
    try {
      ...
      const fromId = selectedOrigin?.id || fromName.toLowerCase().replace(/\s+/g, '-');
      const toId = selectedTarget?.id || toName.toLowerCase().replace(/\s+/g, '-');
      saveRecentRouteSearch(fromId, toId, fromName, toName);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const searchIndex = useMemo(() => {
    ...
    attractions.forEach(a => {
      const destName = a.village_name || a.district || 'Hills';
      index.push({
        id: a.id,
        name: a.name,
        category: 'Attraction',
        location: `${destName}, ${a.district || ''}`,
        image: a.image || 'https://ijyzcvzaosfimvrzgopb.supabase.co/storage/v1/object/public/attractions/View%20Point%20(1).png',
    ...
    // 3. Add Homestays
    homestays.forEach(h => {
      const destName = h.village_name || h.district || 'Remote Village';
      index.push({
        id: h.id,
        name: h.name,
        category: 'Homestay',
        location: `${destName} (Starting from ₹${h.priceMin})`,
        image: (h.images && h.images[0]) || 'undefined',
    ...
    const travelerProfiles: SearchIndexItem[] = [
      {
        ...
        image: "https://api.dicebear.com/7.x/initials/svg?seed=Driver1",
      ...
      },
      {
        ...
        image: "https://api.dicebear.com/7.x/initials/svg?seed=Driver2",
