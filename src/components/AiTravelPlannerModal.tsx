...
import SmartPlannerDashboard from './SmartPlannerDashboard';
import { TripPlan } from '../services/planner/SmartPlannerEngine';
...
export default function AiTravelPlannerModal({
  ...
  const [smartPlan, setSmartPlan] = useState<TripPlan | null>(null);
  ...
  const triggerPlannerAI = async (finalData = formData) => {
    ...
    try {
      ...
      const requestBody = {
        startingLocation: finalData.source || 'Siliguri / NJP',
        days: finalData.days || '3',
        budget: finalData.budget || 'Moderate',
        month: finalData.month || 'October',
        tripType: finalData.tripType || 'Couple',
        stayPreference: finalData.stayPreference || 'Homestay',
  ...
  const handleModifyTrip = async (action: string) => {
    if (!smartPlan) return;
    try {
      const response = await fetch('/api/ai-assistant/modify-trip', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ currentPlan: smartPlan, action })
      });
      const data = await response.json();
      if (data.plan) {
        setSmartPlan(data.plan);
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-white shrink-0">
            <div className="flex items-center gap-2">
              ...
              <div className="w-8 h-8 rounded-full bg-emerald-50 border border-emerald-200 overflow-hidden flex items-center justify-center shrink-0">
                <HillyV1Mascot pose="standing" size="sm" isAnimated={false} showNameTag={false} />
              </div>
              <h2 className="text-sm font-black tracking-tight text-slate-800">Hilly • Official Himalayan Guide</h2>
            </div>
                    ...
                    {isLoading ? (
                      <div className="space-y-6 w-full max-w-sm">
                        <div className="relative mx-auto w-24 h-28 flex items-center justify-center">
                          <HillyV1Mascot pose="reading_map" size="md" isAnimated={true} showNameTag={true} nameTagLabel="Hilly is planning..." />
                        </div>

                        <div className="space-y-2">
                          <h3 className="text-lg font-black text-slate-900 tracking-tight">Hilly is curating your personalized itinerary...</h3>
                        </div>
                      ...
                      /* RENDER DATABASE-FIRST SMART PLANNER DASHBOARD */
                      <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="text-left w-full"
                      >
                        <SmartPlannerDashboard
                          plan={smartPlan}
                          onModifyTrip={handleModifyTrip}
                          onResetWizard={resetPlanner}
                        />
                        <div ref={resultsEndRef} />
                      </motion.div>
                    ) : (
                      /* RENDER COMPLETED ITINERARY MARKDOWN (FALLBACK) */
                      <motion.div
