...
import { firestoreDb, isFirestoreOnline, supabase, supabaseAdmin, isSupabaseOnline, googleFirestoreDb } from './db';
    ...
    setImmediate(async () => {
      try {
        ...
        if (supabaseAdmin && isSupabaseOnline) {
          try {
            const { error } = await supabaseAdmin.from('searches').upsert(event);
  ...
  public async logInteractionAsync(event: InteractionEvent): Promise<void> {
    ...
    setImmediate(async () => {
      try {
        ...
        if (supabaseAdmin && isSupabaseOnline) {
          try {
            const { error } = await supabaseAdmin.from('interactions').upsert(event);
            if (error) throw error;
          } catch (e: any) {
  ...
  public async logUserAnalyticsEvent(
    ...
    setImmediate(async () => {
      try {
        ...
        if (supabaseAdmin && isSupabaseOnline) {
          try {
            const { error } = await supabaseAdmin.from('user_analytics').upsert(event);
            if (error) {
