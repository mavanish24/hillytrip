...
import { dbStore, getDistanceInKm as dbGetDistanceInKm } from './db';
...
// Distance helper - delegates to authoritative server helper db.getDistanceInKm
export function getDistanceInKm(lat1: number, lon1: number, lat2: number, lon2: number): number {
  const d = dbGetDistanceInKm(lat1, lon1, lat2, lon2);
  return d === Infinity || isNaN(d) ? 0 : Number(d.toFixed(2));
}
...
export async function recalculateAllSpatialRelations(): Promise<{ count: number; report?: any }> {
  ...
  for (const d of destinations) {
    ...
    (d as any).nearbyAttractions = legacyNearbyAttractions;
  }
  ...
  for (const a of attractions) {
    ...
    if (nearbyDests.length > 0) {
      ...
      (a as any).nearestDestinationId = primary.record.id;
      (a as any).distanceFromDestination = primary.distance;
  ...
  for (const d of destinations) {
    ...
    (d as any).nearbyDestinations = legacyNearbyDests;
  }
  ...
  for (const h of homestays) {
    ...
    if (nearbyDests.length > 0) {
      ...
      (h as any).nearestDestinationId = closest.record.id;
      (h as any).distanceFromDestination = dist;
    } else {
  ...
  for (const d of destinations) {
    ...
    (d as any).nearbyHomestays = legacyNearbyHomestays;
  }
            ...
            properties: {
              ...
              attractions: {
                ...
                items: {
                  ...
                  properties: {
                    name: { type: Type.STRING },
                    category: { type: Type.STRING, description: "Must be exactly one of the 24 fixed Attraction Master Categories: Mountain & Peaks, Viewpoints & Scenic Points, Waterfalls, Rivers & Lakes, Forests & Nature, Wildlife & Sanctuaries, Tea Gardens, Gardens & Parks, Trekking & Hiking, Camping & Adventure, Temples & Shrines, Monasteries, Churches & Religious Sites, Heritage & Historical, Villages & Local Life, Scenic Roads & Passes, Bridges & Infrastructure, Railways & Stations, Caves & Rock Formations, Hot Springs & Wellness, Beaches & Riverfronts, Culture & Experiences, Food & Local Markets, Other Attractions" },
