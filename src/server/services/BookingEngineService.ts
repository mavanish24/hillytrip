...
import { NotificationEventType } from '../../types/communication';
...
BookingEventBus.subscribe(async (payload) => {
  ...
  const eventTypeMap: Record<string, NotificationEventType> = {
    ...
    'booking.reserved': 'BOOKING_ASSIGNED',
    'booking.confirmed': 'BOOKING_CONFIRMED',
    'booking.cancelled': 'BOOKING_CANCELLED',
    'booking.expired': 'BOOKING_EXPIRED',
    'booking.completed': 'TRIP_COMPLETED',
    'booking.status_updated': 'BOOKING_ASSIGNED'
  };

  const commEventType: NotificationEventType = eventTypeMap[event] || 'BOOKING_CREATED';

  // Publish to Communication Engine for multi-channel processing
  try {
    const l = lead as any;
    const customerIdentifier = l.customerEmail || l.customerMobile || 'customer';
    await communicationEngine.publishEvent({
      eventId: payload.eventId,
      eventType: commEventType,
      recipientId: customerIdentifier,
      recipientEmail: l.customerEmail,
      recipientPhone: l.customerMobile,
      recipientRole: 'customer',
      variables: {
        booking_id: bookingId,
        customer_name: l.customerName || 'Valued Guest',
        pickup: l.pickupLocation || l.pickup || 'Pickup Location',
        destination: l.destinationLocation || l.destination || 'Destination',
        journey_date: l.travelDate || l.travel_date || 'Selected Date',
        vehicle: l.selectedVehicleCategory || l.vehicle || 'Taxi',
        quote_price: l.estimatedTotalFare || l.amount || 0,
        operator_name: l.assignedPartnerName || 'HillyTrip Operator'
      }
    });

    if (l.assignedPartnerId) {
      await communicationEngine.publishEvent({
        eventId: `${payload.eventId}-operator`,
        eventType: commEventType,
        recipientId: l.assignedPartnerId,
    ...
    console.error('Failed to forward event to Communication Engine:', err);
  }
});
