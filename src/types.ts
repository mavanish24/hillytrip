export { ATTRACTION_CATEGORIES_MASTER, type AttractionMasterCategory, type AttractionCategoryMetadata } from './constants/attractionCategories';
...
export interface TaxiStand {
  ...
  name: string;
  slug?: string;
  type: 'main_hub' | 'sightseeing_hub' | 'sub_hub' | 'local_stand';
  latitude: number;
  longitude: number;
  district?: string;
  state?: string;
...
export interface Destination {
  id: string; // matches Slug, ID or village_code
  name: string;
  slug?: string;
  village_code?: string;
  village_name?: string;
...
export interface Attraction {
  ...
  name: string;
  slug?: string;
  category: 'Viewpoint' | 'Monastery' | 'Waterfall' | 'Lake' | 'Trek' | 'Village' | string;
...
export type ClaimStatus = 'UNCLAIMED' | 'CLAIMED' | 'SUSPENDED';
export type ClaimSource = 'Homepage' | 'Listing Page' | 'Search' | 'Direct Link' | 'QR Code';
export type ClaimPlatform = 'Web' | 'Android' | 'iOS';

export interface BusinessClaimFields {
  ...
  businessType?: 'Homestay' | 'Hotel' | 'Resort' | 'Taxi Operator' | 'Restaurant' | 'Travel Guide' | 'Vehicle Rental' | 'Campsite' | 'Tour Operator' | string;
}

export interface Homestay extends BusinessClaimFields {
  id: string;
  name: string;
  slug?: string;
  destinationId?: string;
...
export interface ClaimRequest {
  ...
  homestayId: string;
  listingId?: string;
  listingType?: 'Homestay' | 'Hotel' | 'Resort' | 'Taxi Operator' | 'Restaurant' | 'Travel Guide' | 'Vehicle Rental' | 'Campsite' | 'Tour Operator' | string;
  ...
  status: 'pending' | 'approved' | 'rejected' | 'instant_verified';
  adminRemarks?: string;
  createdAt: string;
  verifiedAt?: string;
...
export interface ClaimReminderLog {
  ...
  milestone: 'Day 0' | 'Day 7' | 'Day 15' | 'Day 30';
  milestoneDay: number;
  subject: string;
  message: string;
  sentAt: string;
...
export interface BookingLead {
  ...
  // Extra fields for booking engine & Payment Engine
  ...
  // Phase 3A/3B: Structured Settlement Fields
  payoutAmount?: number;
  utrNumber?: string;
  payoutDate?: string;
  payoutMethod?: 'NEFT' | 'RTGS' | 'IMPS' | 'UPI' | 'Manual Bank Transfer' | string;
...
export interface Membership {
  ...
  plan_id: string;
  price: number;
  status: 'ACTIVE' | 'EXPIRED' | 'CANCELLED' | 'PENDING' | 'active' | 'expired' | 'cancelled' | 'pending';
  trial: boolean;
  started_at: string;
  expires_at?: string;
  payment_id?: string;
...
/**
 * Helper to determine if a homestay is verified and eligible for public display across village pages,
 * destination pages, search results, recommendations, and catalogs.
 * Unclaimed or unverified homestays return false and are hidden from public listings.
 */
export function isHomestayPublic(h: Partial<Homestay> | null | undefined): boolean {
  if (!h) return false;

  const status = (h.status || '').toLowerCase();
  const claimStatus = (h.claim_status || '').toLowerCase();

  // Unclaimed or explicitly pending/rejected/suspended homestays must NOT appear on public pages
  if (
  ...
  // Default fallback check for verified/claimed records
  ...
  const isClaimedOrVerified =
    ...
    h.isVerified === true ||
    h.verified === true ||
    Boolean(h.ownerId && h.ownerId.trim() !== '') ||
    Boolean(h.owner_user_id && h.owner_user_id.trim() !== '');

  return isApproved && isClaimedOrVerified;
}

// ==========================================
// PHASE 3A: OWNER PAYOUT PROFILE TYPES
// ==========================================
export type PayoutDetailsStatus = 'NOT_SUBMITTED' | 'PENDING_VERIFICATION' | 'VERIFIED' | 'REJECTED';
...
export interface OwnerPayoutProfile {
  ...
  ifsc: string;
  upiId?: string;
  payoutDetailsStatus: PayoutDetailsStatus;
  rejectionReason?: string;
  verifiedAt?: string;
  verifiedBy?: string;
  createdAt: string;
...
export interface MaskedOwnerPayoutProfile {
  id?: string;
  ownerUserId?: string;
  homestayId?: string;
  accountHolderName?: string;
