...
-- ----------------------------------------------------------------------------
-- 10. Table: blogs
-- Publishing system for travel articles, regional guides, and updates.
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.blogs (
    ...
    status VARCHAR(50) NOT NULL DEFAULT 'Draft', -- 'Draft', 'Under Review', 'Approved', 'Scheduled', 'Published', 'Archived'
...
CREATE INDEX IF NOT EXISTS idx_blogs_destination ON public.blogs(destination_id);
...
-- 7. Blogs Policies
CREATE POLICY select_blogs_policy ON public.blogs
    FOR SELECT USING (TRUE); -- Public read access for blogs

CREATE POLICY write_blogs_policy ON public.blogs
    FOR ALL USING (TRUE);
